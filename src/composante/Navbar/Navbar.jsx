...
const Navbar = () => {
  ...
  const categories = [
    { name: "All", icon: "🍽️", path: "/all" },
    { name: "Burger", icon: "🍔", path: "/burger" },
    { name: "Plate", icon: "🍛", path: "/plate" },
    { name: "Dessert", icon: "🍰", path: "/dessert" },
    { name: "Pasta", icon: "🍝", path: "/pasta" },
    {
      name: "Moroccan Food",
  ...
  return (
    <nav className="bg-white shadow-md px-8 py-4 flex justify-between items-center">
      ...
      {dropdownOpen && (
        ...
        >
          {categories.map((category, index) => (
            <li key={index} className="px-4 py-2 hover:bg-[#FD4C2A] hover:text-white rounded flex items-center gap-2">
              ...
              <Link to="/Our_Menu">{category.name}</Link>
            </li>
          ))}
        </ul>
      )}
        ...
        <li>
          <Link to="/Our_Menu" className="relative px-4 py-2 text-black hover:text-[#FD4C2A]">Menu</Link>
        </li>
        <li>
